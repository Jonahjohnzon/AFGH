...
import { secondopen } from "../feature/menu/slice";
...
const Slide = () => {
    {
      ...
      const menu = useSelector((state) => state.menu.secondopen);
  ...
  return (
    ...
    {menu && (
      <motion.div
        ...
        animate="animate"
        exit="exit"
        className=" w-[100vw] h-[100vh] fixed top-0 z-40  text-white font-barlow"
      >
        <div
          className="bg-dak bg-opacity-60  w-full h-full"
          onClick={() => dispatch(secondopen(false))}
        ></div>
        <motion.div
          variants={child}
          className=" fixed w-[320px] bg-dak h-full right-0 top-0 z-50 flex flex-col items-center justify-between"
        >
          <div className=" w-[80%]">
            ...
            <form className=" mb-[3vh]">
              <div className="flex item">
                <input type="email" className=" bg-transparent border-solid border-[0.01em] w-full h-14 rounded-s-md border-gray pl-5 " placeholder="Search"/>
                <div className=" border-solid border-[0.1em] px-5 flex items-center flex-1 rounded-e-md border-gray"><BiSearch size={20} className=" hover:text-cream cursor-pointer transition-all duration-500"/></div>
              </div>
            </form>
            <div>
            ...
            <div className=" text-green font-bold mt-[3vh] mb-[3vh]">
              <EffectText words={"LET'S LINK UP"} />
            </div>
            <div className=" h-[0.1px] opacity-25 w-full bg-gray mb-[3vh]"></div>
